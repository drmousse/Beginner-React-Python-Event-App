...
# classes
class EventGameComment(TioDB.db.Model):
    __tablename__ = 'tio_event_game_comment'
    __classname__ = 'tio_event_game_comment'
    ...
    comment = TioDB.db.Column(String(3000))

    @classmethod
    def ObjectByPK(cls, event_id, game_id):
        try:
            return TioDB.db.session.execute(TioDB.db.select(cls).filter_by(event_id=event_id, game_id=game_id)).scalar_one()
