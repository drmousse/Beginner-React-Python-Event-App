...
# classes
class OLC(TioDB.db.Model):
    __tablename__ = 'tio_olc'
    __classname__ = 'tio_olc'

    status = TioDB.db.Column(Integer, primary_key=True)
    type = TioDB.db.Column(String(tioglobals.LENGTH_STATUS_TYPE), primary_key=True)
    color = TioDB.db.Column(String(15), nullable=False)
    name = TioDB.db.Column(String(20), nullable=False)
